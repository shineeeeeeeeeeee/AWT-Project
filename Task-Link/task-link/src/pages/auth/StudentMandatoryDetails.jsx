...
function StudentMandatoryDetails() 
{
  ...
  const [formData, setFormData] = useState({ firstName: "", middleName: "", lastName: "", country: "", address: "", phone: "" });
  ...
  const handleResumeChange = (event) => {
    ...
    {
      setResumeFile(null);
      setErrors((prev) => ({ ...prev, resumeFile: "Please upload a valid PDF file." }));
    }
  };
  ...
  const handleSubmit = (event) => {
    ...
    console.log(payload);
    // This Payload must be posted in Student DB.
      ...
      <main className="std-main">
        <div className="info-strip" role="note">
          ...
          <div className="info-text">
            You can view and edit your profile details here. Any changes you make will be reflected
            in all your <strong>active job applications</strong>.
          </div>
        </div>
          ...
          <div className="resume-box upload-box">
            ...
            <div className="resume-cta">
              ...
              <div className="file-name">
                {resumeFile ? (resumeFile.name) : (<span className="muted">No file selected</span>)}
              </div>
            </div>
            {errors.resumeFile && <div className="error-msg">&#9888; {errors.resumeFile}</div>}
          </div>
...
{/* -----------------------------------------------------------------SKILLS---------------------------------------------------------- */}
          ...
          <div style={{ marginBottom: 14 }}>
            <div className="muted">
              &nbsp;&nbsp; Add the skills you want recruiters to see — click the + to add.
            </div>
          </div>
...
{/* -----------------------------------------------------------------SUBMIT---------------------------------------------------------- */}
          ...
          <div className="std-actions" style={{ marginTop: 25 }}>
            <button type="submit" className={`primary-btn ${Object.keys(errors).length === 0 ? "active" : ""}`}> Submit </button>
            <button type="button" className="secondary-btn" onClick={() => navigate("/fill_student_details")}> Cancel </button>
          </div>
        </form>
      </main>
{/* --------------------------------------------------------------------------------------------------------------------------------- */}
